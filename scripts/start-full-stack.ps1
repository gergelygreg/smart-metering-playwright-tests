...
try {
    ...
    if ($NoBuild) {
        ...
        & wsl.exe @composeArgs up -d --build
    }
    ...
    Write-Host ""
    Write-Host "============================================"
    Write-Host "SMART METERING FULL DOCKER STACK IS READY"
    Write-Host "============================================"
    ...
    try {
        & wsl.exe -d $Distro -- docker compose -f $ComposeFile logs --no-color --tail 150
    }
    catch {
    }
